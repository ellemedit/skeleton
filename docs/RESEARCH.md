...
업그레이드 경로: 동일한 골격/활성화 인터페이스를 유지한 채, 추후 사실적 메시로 교체 가능(설계상 분리됨).

### 2.3 리깅 인체 모델 (최종 채택)

절차적 프리미티브는 "사람다움 + 섬세한 근육 표현"에 한계가 있어, 리깅된 **인체 메시**로 전환했다.

- **리소스 조사**: 네트워크 정책상 `raw.githubusercontent.com`은 접근 가능(jsDelivr·threejs.org는 차단). three.js 예제의 리깅 휴먼 GLB(**Xbot**, Mixamo, ~20k 삼각형, 67본)와 Khronos 샘플(CesiumMan CC-BY 등)을 입수 가능. **무료로 리깅된 근육 해부(écorché) 모델은 발견되지 않음** → 실제 근육 메시는 대개 정적 자산.
- **채택**: 리깅 인체 GLB(사람 형상) + **피부 표면 근육 히트맵**(정점→근육군 분류, 활성도→발광). 사람다움·동작·부위 강조를 동시에 확보하되, 근육은 별도 메시가 아닌 표면 하이라이트로 표현.
- **포즈 재사용**: 기존 절차적 리그(검증된 3대 운동 포즈)를 인체 스켈레톤으로 **리타게팅**해 재사용한다. 다리·몸통은 바인드가 호환되어 월드-델타 리타게팅이 잘 되고, 팔(A↔T 포즈 차이)은 운동별로 직접 저작한다.
- **렌더링**: 브라우저 데모는 WebGL(GPU 스키닝). 헤드리스(이 환경)는 WebGL2/Chromium이 없어 three GLTFLoader로 로드 후 **CPU 선형 블렌드 스키닝 + 소프트웨어 래스터라이저**로 렌더링.
